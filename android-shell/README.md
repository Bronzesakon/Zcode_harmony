...
| P8 | **多工作区 / 多任务订阅**（2026-09-16 定的下一个方向） | **未验证，只做了理论推导**：代码侧已具备（多桥并发、`maxWorkspaces=12`、正文与卡片按 `(workspaceKey, sessionId)` 键），只差 `Tier2Probe.startCoverage()` 那层"只开页面当前工作区"的保守过滤——它建立在一条**已被推翻的归因**上。桌面端能否同时服务多工作区＝未测（旧证据被 host 之死污染）；流体云仍只提升 2 张卡（`PromotionPolicy.MAX_PROMOTED`） | 见 `docs/17-多工作区多任务订阅-理论推导.md`；**第一步＝E1**（「下一步」0） |
...
真机当前跑的是**本机出的 `1.0.0-local.145`**（本地 keystore 同签名，`adb install -r` 覆盖安装，
**零 CI 消耗**；v130–v145 都是 2026-09-16 这一夜为"后台 60 秒墙"做的实验件）。`pre` 分支与滚动预发布
`android-pre` 停在 **`764235e`**；本夜的本地提交是 `8e1356a / 28ff38c / 537529f / 9222777 / f525107 / f02a140`
（只读壳那轮是 `c8c93e2`、收尾文档 `c5f9407`），**全部仅本地、未推送**；
`docs/15·16·17-…md` 与根目录历史 APK 仍未跟踪。
...
**下一个方向（2026-09-16 收尾时定，只做了理论推导、未动代码）**：**多工作区 / 多任务对话订阅**。
详见 **`docs/17-多工作区多任务订阅-理论推导.md`**，要点：**代码侧已经具备**（多桥并发
`bridges`/`bridgesById` + `maxWorkspaces=12`；正文与卡片按 `(workspaceKey, sessionId)` 键；
外壳轮询本来就遍历全部在跑任务），**唯一挡路的是 `Tier2Probe.startCoverage()` 里我加的那层
"只开页面当前工作区"的保守过滤**——它建立在一条**已被推翻的归因**上（见第 3 条）。
真正没验过的只有"桌面端能否同时服务多个工作区的桥"，而且手上那组"1 成功 6 被拒"的旧证据**被
...
5. **【下一个方向】多工作区 / 多任务订阅**：只做了理论推导，见 `docs/17-多工作区多任务订阅-理论推导.md`；
   第一步＝「下一步」0 的 E1。
