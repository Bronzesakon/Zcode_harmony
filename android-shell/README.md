...
`pre` @ `37c1880`（本轮从 `a3a57f5` 起共 12 个提交，**每次推送都触发 CI**；其中三次是失败轮——资源链接、测试里 const、动态配色 lambda 元数——都按 CI 给出的错误修好后才转绿，失败与修法见阶段 1/5）；真机最后一版装的是 **`1.0.0-pre.43`**（02:25 装的；02:27 设备掉线后又有两版修复没能装上去，最新版在滚动预发布里）（之后 04:00 前又推了两版修复，但设备已掉线装不上）；验收记录见「阶段验收记录」，阻塞项见「拦路石」。**第八轮结束时的状态**：CI 全绿（JS 50 + Kotlin 42 单测），
