...
`pre` @ `37c1880`（本轮从 `a3a57f5` 起共 12 个提交，**每次推送都触发 CI**；其中三次是失败轮——资源链接、测试里 const、动态配色 lambda 元数——都按 CI 给出的错误修好后才转绿，失败与修法见阶段 1/5）；真机最后一版装的是 **`1.0.0-pre.43`**（02:25 装的；02:27 设备掉线后再没装上新版）。**滚动预发布里那一版是最新的**：最后一个动 `app/` 的提交是 `e52a7c4`，其后全是文档提交（之后 04:00 前又推了两版修复，但设备已掉线装不上）；验收记录见「阶段验收记录」，阻塞项见「拦路石」。**第八轮结束时的状态**：CI 全绿（JS 50 + Kotlin 42 单测），
...
| # | 用户原话（摘） | 验收口径 | 状态 |
| --- | --- | --- | --- |
| B1 | 「按照 miuix 重构设置页」 | 设置页按 MiuiX 设计语言重排（规格取自本机 `docs/miuix` 源码） | **结构已在真机验证**（度量逐项对表），观感待截图 |
...
| B5 | 「设置底部的手势横条沉浸，网页直接底部对齐屏幕底部边距，导航条原位置悬浮在底部即可」 | 底部 inset 不消费（键盘除外）：网页贴到屏幕底边，手势条悬浮其上 | **真机已验证**（WebView 底边 = 2800px = 屏幕底边）；键盘上抬待复验 |
...
| B7 | 「图标都去找 Android 的原生图标库不要自己瞎画」 | 用 Google 官方 Material Icons 字形（`ic_shortcut_scan` / `ic_shortcut_settings`），路径逐字照抄 | **真机已验证**（图标资源已挂上）；外观待截图 |
...
| C2 | 「任务完成后，流体云立即显式从屏幕顶端的横条弹出展开为方框，强提示任务已完成（即把运行中状态改为已完成三个字标记然后其他按正常通知内容来）」 | 完成瞬间额外发一张 promoted（ongoing）卡片，标题 `已完成 · 任务名`，正文照旧；15 秒后自动收 | **待触发**（需要有一件任务在观测期内完成；机制同 C1 已验证的 promoted 路径） |
...
| # | 要求 | 状态 |
| --- | --- | --- |
| D1 | 「全程走 github 的 pre 分支推送触发的 ci 流程编译，从 release 下载安装并调试形成闭环，切勿本地编译」 | **已走通一轮**（pre.42 从 `android-pre` 直链下载安装） |
| D2 | 「adb bridge 可用，192.168.0.185:45903 链接网络 adb，屏幕常亮，禁止息屏测试因为会困在锁屏状态」 | 遵守中（用 `mcp__adb-bridge__*`，显式传 target） |
| D3 | 「可以退出应用回到后台测试」 | 遵守中 |
| D4 | 「每推进一个阶段的任务都要跑通 ci 构建和测试验证，正好做提交的快照管理」 | **阶段 1 完成**，详见「阶段验收记录」 |
