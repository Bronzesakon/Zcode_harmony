...
1. ~~把后台存活测满 30 分钟~~ **已完成**（pre.39，29 分 26 秒 → 保活成立）。下一件真机事是「已知问题 C」的取舍，
   或「项目现状」里 P1/P3/P4 那些从未验证过的项（键盘上抬 / 悬浮弹窗 / 流体云 / 上传与通知细节）。
...
第八轮即本轮（pre.34–pre.39：**后台存活测满 29 分 26 秒并判定保活成立**、页面重连的四个假设全部否定、
修掉会骗人的 close 归因、新增后台三把尺子）——每轮都是就地改写。
