...
| P5 | 标题回退/内容不加载（用户 2026-09-12 两次报） | **「3s 双态直刷」已实现（待真机验收）**：进对话 +3s 查一次，头部回退「新建任务」（状态 A）或输入框灰/禁用（状态 B）→ 直接刷新；唯一保留 15s 最小间隔防死循环，其余守卫按用户指示全弃 | 见「已知问题 D·1 / D1-b」与 [`标题迟加载-调研报告`](标题迟加载-调研报告-2026-09-12.md) |
...
### 已知问题 F：注入层偶发整体失效——滚动条回归原生 + 状态栏退回 boot 底色（2026-09-12，已加双保险自愈，根因待定案）

现象（用户外场报告）：某些情况下页面「回退」到右侧自带 14px 滚动条（零宽样式失效）**且**状态栏
取色不对（页面状态不再上报，原生侧停在 boot 底色）；页面自身照常可用（浏览/发消息正常）；
重启客户端后恢复。git 历史两版滚动条：`fd05a69` 零宽 v1 → `43c6998` 全撤销 → `18968b1`
方案 A 悬浮自绘（现行）。

**定性**：两症状同时出现，最强指向是**那一次页面加载里注入层整体没生效、或运行期被整体拆掉**——
页面自己有 WebSocket，注入层缺席也能照常浏览，但零宽 CSS、页面状态上报、壳桥接（后台通知）全缺席。
activity 重建路径已排除（onCreate 每次 `applyUrl → installInjection`，13:05–13:11 日志每次加载都有
「已安装 document-start 注入」）；坏窗口恰好不在日志里，根因未定案。

**已落地的双保险兜底**（本节即"怎么样能全程确保悬浮自绘 + 取色不异常"的答案）：

1. **页面内自愈**（inject.js）：每次心跳（10s）检查 `style[data-zcode-shell="scrollbar-width"]`
   是否还挂在文档里，被移除/未装上就重装并留 warn「检测到滚动条置零样式丢失，已重新安装（自愈）」；
   悬浮指示条节点若脱离文档，下一次滚动自动重挂。
2. **原生看门狗**（MainActivity）：健康加载里 document-start 一定在一两秒内上报页面状态；首次加载
   完成后 **10s 零上报 = 注入缺失** → 自动重载一次（每进程只补一次防循环），日志
   「页面加载 10s 未收到注入层状态上报，疑似注入缺失，自动重载一次」。

判读：坏窗口出现后 ≤10–15s 应看到自愈并恢复。若看门狗触发了重载 → 「注入整体缺失」形态
（查 installInjection 的资产读取与 URL 规则匹配）；若只是样式重装 → 「运行期被移除」形态。
下次坏窗口的日志会给出归属，再修根因。
...
1. **标题回退/内容不加载：「3s 双态直刷」已实现，待真机验收**。口径（用户定）：进对话信标
   （subscribe 或 rows-range）后 **+3s 查一次**——状态 A 头部回退「新建任务」、状态 B 输入框灰/禁用
   （有标题但内容没加载）→ 直接刷新；唯一保留 15s 最小间隔（防死循环的终止性必需），其余守卫全弃。
   验收：覆盖安装最新 pre → 冷状态首次打开会话 → 正常 9–22s 自愈不触发；卡住则 3s 刷新 + 日志
   `进对话 3s 未就绪（…），刷新页面`。⚠️ DOM 判据（头部文本/输入框 disabled）待真机核一次。
...
| 快速刷新干活没 | 卡住时 `进对话 3s 未就绪（标题回退/输入框未就绪），刷新页面`；健康 `3s 检查：…均就绪` |
| 注入层失效了没（滚动条/取色回退） | `检测到滚动条置零样式丢失…（自愈）`；原生 `疑似注入缺失，自动重载一次` |
