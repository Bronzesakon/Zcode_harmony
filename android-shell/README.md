...
| P5 | 标题回退「新建任务」（用户 2026-09-12 两次报） | **已定位到窗口，修法待用户拍板**：桌面端还没加载这条会话时 `readSession` 被拒 → 首次打开回退，之后自愈（实测 9–22 秒，忙时约 2 分钟；**再打开一次 <2 秒即正确**）。责任在页面 + 桌面端，壳侧无干净杠杆 | 见「已知问题 D·1 / D1-b」与 [`标题迟加载-调研报告`](标题迟加载-调研报告-2026-09-12.md) |
...
`pre` @ `506c0dd`；**最后一个改动 `app/` 的提交是 `9d79c4e`**（其后全是文档），
CI 全绿，滚动预发布里那一版就是含全部代码的版本；真机装的是 **`1.0.0-pre.50`**。
...
1. **标题迟加载 → 等用户拍板选哪条修法**。已定位：冷会话首次打开回退、热会话秒出，
   窗口 = 桌面端何时愿意答 `readSession`（实测 9–22 秒，忙时约 2 分钟）。
   壳侧没有干净杠杆；方案 (c)（读 DOM 判回退 + 再点一次任务行）会动网页 UI，**未实现，等用户点头**。
   ⚠️ 别重新取证——[`标题迟加载-调研报告`](标题迟加载-调研报告-2026-09-12.md) 里连复现命令和判据都写好了。
