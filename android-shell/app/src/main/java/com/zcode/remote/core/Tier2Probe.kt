...
object Tier2Probe {
     ...
     * **卡片侧不设上限**（2026-09-18 用户否决硬上限）：在跑的任务有几个就提升几个；
     * 只有"留到用户清掉"的完成卡有界限（`PromotionPolicy.MAX_FINISHED_PROMOTED`）。
     * 曾经以为"ColorOS 只并发提升 2 张"，被现场推翻（三张卡同时显示、dump 里三条 `PROMOTED_ONGOING`）。
