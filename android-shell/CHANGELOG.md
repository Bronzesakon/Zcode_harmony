...
- 真机验收（一加 PLC110 / ColorOS 16 / API 36 / WebView 154，`1.0.0-pre.42` → `pre.50`）：
  - **日志与视图树**（设备锁屏期间）：状态栏三态换色、`WebView 0,141-1272,2800`（顶边=状态栏高、底边=屏幕底边）、两条快捷方式已注册、设置页度量逐项对上规格且无崩溃、通知标题与 `PROMOTED_ONGOING` 正确、fault 收敛（reopen 0 次 vs 旧版 22 次）。
  - **截图**（用户插 USB、设备解锁后）：三种状态下状态栏带子都与网页顶面同色、**无缝**；无应用栏；网页贴底、手势条悬浮其上；**流体云真的出卡**（标题 `运行中 · 任务名`、正文只有进展、状态栏还有「运行中」芯片）；MiuiX 设置页观感与规格一致（且开关是 MiuiX 蓝，说明排除动态配色生效）；长按菜单两条快捷方式与 Material 字形图标；点进任务会话头显示的是**任务名**而非「新建对话」（与 D1 定位吻合）。
  - **A2 长后台复验**：退后台 6 分 59 秒（41 次后台心跳、284 帧、`保活成立`，P0 未回归）；窗口内 relay 自断两次，两条连接各只 fault 一次即收手，`reopening` 全程 0 次。
  - 顺手抓到并修掉一个只有真机端到端能发现的回归：标题加状态前缀后，`EXTRA_TASK_TITLE` 里塞的仍是被装饰过的标题，导致「点通知跳到任务」静默失效。
  - 未做：键盘上抬未重测（上一轮已验证、且本轮改动不触及键盘弹起时的数值）；`已完成` 卡片尚未被一次真实完成触发（机制与 C1 同一条 promoted 路径，另有单测）。
