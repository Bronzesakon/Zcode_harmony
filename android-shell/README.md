...
`pre` @ `d9ce891`（本轮从 `a3a57f5` 起共 13 个提交，**每次推送都触发 CI**；其中三次是失败轮
...
| D2 | 「adb bridge 可用，192.168.0.185:45903 链接网络 adb，屏幕常亮，禁止息屏测试因为会困在锁屏状态」 | **已中断**：接手时设备就已息屏且处于安全锁屏；02:27 息屏超时到点后设备掉出 Wi-Fi（详见「拦路石」）。用 `mcp__adb-bridge__*` 时始终显式传 target |
